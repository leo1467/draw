...
def split_testIRR_draw(split,fileName):
    ...
    #設定bar屬性
    barColorSet = ['steelblue', 'darkorange', 'pleturquoise', 'wheat', 'lightcyan', 'lightyellow']
    totalBarWidth = 0.85
    singleBarWidth = 0.20
    #tables資料
    tables = []
    ...
    for Fileindex, file in enumerate(allIRRFile):
        ...
        if file.split('.')[1] == 'csv':
            ...
            for splitIndex in range(len(dfCuttedIndex) - 1):
                #將過長的df切一半
                df1 = df.iloc[dfCuttedIndex[splitIndex]: dfCuttedIndex[splitIndex + 1]]
                #plot bar
                ax = df1.plot.bar(width = barWidth, rot = 0, color = colorDict, edgecolor = 'black', linewidth = 0.2)
                ax.legend(loc = 'upper center', bbox_to_anchor = (0.5, -0.2), fancybox = True, shadow = False, ncol = 6, fontsize = 7)
                ax.yaxis.set_major_formatter(mtick.PercentFormatter())  #把座標變成%
                x = np.arange(len(df1.index))
                plt.xticks(x, df1.window, rotation = 45, fontsize = 7)
                plt.grid(axis = 'y')
                #設定lable顏色
                for cellIndex in ax.get_xticklabels():
                    ...
                    for slideGroup in slidingLableClrList:
                        if txt in slideGroup[0]:
                            ...
                            break
                topTable = plt.table(colLabels = tableDf.columns, cellText = tableDf.values, loc = 'top', cellLoc = 'center')
                topTable.auto_set_font_size(False)
                topTable.set_fontsize(6.5)
                if len(df.columns) > 5:
                    if splitIndex == 0:
                    ...
                    table = plt.table(cellText = chooseDf.values, 
                              loc = 'bottom', cellLoc = 'center', 
                              rowLabels = [chooseDf.index[0], chooseDf.index[1]],
                              bbox=[0.0, -0.6, 1, 0.28])
                    
                plt.title(company + '_' + techName + '_IRR_rank ' + str(splitIndex), pad = 30)
                plt.savefig(company + '_' + str(splitIndex) + '.png', dpi = 300, bbox_inches = 'tight')
                plt.cla()
                plt.close()
        # exit(0)
    os.chdir(owd + '/result/')
    for dfIndex, eachDf in enumerate(tables):
        if dfIndex == 0:
            eachDf.to_csv('tables.csv')
        else:
            eachDf.to_csv('tables.csv', mode = 'a', header = None)
